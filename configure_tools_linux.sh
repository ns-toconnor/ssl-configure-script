...
# shebang) and tells us nothing about the user's actual shell.
get_shell(){
    ...
    case "$login_shell" in
        zsh)
            ...
            ;;
        bash)
            # On most Linux distros ~/.bashrc is sourced for interactive shells;
            # login-shell propagation usually comes via ~/.bash_profile or ~/.profile.
...
try:
    ...
    settings['terminal.integrated.env.linux']['NODE_EXTRA_CA_CERTS'] = cert_path
...
configure_vscode_variant "VS Code" "$HOME/.config/Code/User/settings.json"
configure_vscode_variant "VS Code Insiders" "$HOME/.config/Code - Insiders/User/settings.json"
